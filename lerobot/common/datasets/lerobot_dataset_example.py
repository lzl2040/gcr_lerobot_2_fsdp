...
from lerobot.common.utils.data_utils import save_to_json
    ...
    def __getitem__(self, idx) -> dict:
        ...
        if self.image_transforms is not None:
            ...
            for cam in image_keys:
                ...
                # item[cam] = torch.from_numpy(np.array(item[cam]))
                # print(torch.max(item[cam]))
...
class MultiDatasetforDistTraining(torch.utils.data.Dataset):
    def __init__(self, cfg, image_transforms, seed: int = 1000, 
        ...
        # save_to_json(self.stats, os.path.join("/home/v-wangxiaofa/lzl/gcr_lerobot_2_fsdp/lerobot/stats", f"{cfg.dataset.data_mix}_stats.json"))
        ...
        for key in new_keys:
            ...
            if frame_num > 1:
                if frame_num >= 20:
                    ...
                    # 计算缺失帧数
                    pad_len = 20 - frame_num
                    # 获取图像 shape (3, H, W)
                    C, H, W = img_data.shape[1:]
                    # 构造零帧 (pad_len, 3, H, W)
                    zero_padding = torch.zeros(pad_len, C, H, W, dtype=img_data.dtype)
                    # 拼接在后面
                    item[key] = torch.cat([zero_padding, img_data], dim=0)
                    is_pad_frame[key] = pad_len
    ...
    def save_item(self, item, is_pad_frame):
        # 构建目标文件夹路径
        data_id = len(os.listdir(self.preprocess_root))
        folder_path = os.path.join(self.preprocess_root, f"data_{data_id:010d}")
        ...
        for key in self.new_obs_image_keys:
            ...
            # print(torch.max(img_data), torch.min(img_data)) # unit8
            # torch.save(img_data, os.path.join(folder_path, f'{save_name}.pt'))
